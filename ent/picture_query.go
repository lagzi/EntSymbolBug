...
func (pq *PictureQuery) QueryComponent() *ComponentQuery {
	query := &ComponentQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PictureQuery) Clone() *PictureQuery {
	...
	return &PictureQuery{
		...
		withComponent: pq.withComponent.Clone(),
...
func (pq *PictureQuery) WithComponent(opts ...func(*ComponentQuery)) *PictureQuery {
	query := &ComponentQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withComponent = query
	return pq
}
...
//		GroupBy(picture.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Picture.Query().
//		Select(picture.FieldCreateTime).
//		Scan(ctx, &v)
//
...
func (pq *PictureQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Picture, error) {
	var (
		...
		loadedTypes = [2]bool{
			pq.withUser != nil,
			pq.withComponent != nil,
		}
	)
	if pq.withUser != nil || pq.withComponent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withUser; query != nil {
		...
		for i := range nodes {
			if nodes[i].user_user_picture == nil {
				continue
			}
			fk := *nodes[i].user_user_picture
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_user_picture" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withComponent; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Picture)
		for i := range nodes {
			if nodes[i].component_pictures == nil {
				continue
			}
			fk := *nodes[i].component_pictures
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "component_pictures" returned %v`, n.ID)
			}
			for i := range nodes {
